...
float MSAPID::pid_eval( float current_value, int delta_sign /*= 1 */ )
	...
	if (deltaTime >= dt)
		{
		if (pid_par_offset == 61) //Если регулятор подогрева пропорционально увеличиваем K при перегреве.
			{
